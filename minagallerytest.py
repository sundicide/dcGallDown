...
if __name__ == "__main__":
    ...
    while (pageindex < 2):  # navigate to X Page
        ...
        for tempData in eachATag: #
            try:
                ...
                if prop != None and prop[0] in class_list:  # if class property exist, check icon_pic_n
                    ...
                    parsedData1 = bs2.find_all('div', 's_write')
                    #parsedData = bs2.find_all('ul', 'appending_file')

                    #print(parsedData)#bs4.element.ResultSet
                    parsedData2 = parsedData1[0].find_all('img')
                    for index2, tempInFor in enumerate(parsedData2):

                        filename = ''#print(tempInFor)
                        tempInIf = tempInFor.get('onclick')
                        if tempInIf:

                            tempurl = re.split('[\']',tempInIf)
                            #download_web_image(tempurl, folder_name, index)
                            #index+=1
                            tempurl = tempurl[1]
                    ...
                    '''
                    result=[]
                    for th in parsedData:
                        result.extend(th.find_all("a"))#all of appendedfile append to variable result

                    for temp in result:#distinguish type(jpg,gif) from result
                        file_name = temp.get_text()
                        #print(temp.get('href'))
                        temp2 = temp.get('href')
                        temp2 = temp2.replace("download.php","viewimagePop.php")
                        temp_bs = goIntoUrl(temp2)#making new url for bigsize pic
                        #print(temp_bs.prettify())
                        temp3 = temp_bs.find('img')
                        temp3 = temp3.get('src')
                        #print(temp3)
                        download_web_image(temp3, folder_name, file_name)
                    ...
                    #parsedData type is <class 'bs4.element.ResultSet'>
                    #print(type(parsedData))

                    #splitData = str(parsedData).split('src=')
                    #splitData = str(parsedData).split('onclick')
                    #splitData = str(parsedData).split('>')
                    ...
                    '''
                    indexForFile = 0;
                    for tempData2 in splitData: #getting img URL

                        indexForFile +=1
                        finalData = tempData2.split(' ')
                        imgURL = str(finalData[0])
                        imgURL=imgURL.split('"')
                        imgURL=imgURL[1]
                        ...
                        if 'dcimg1' in imgURL:
                            imgURL = imgURL.replace("dcimg1","dcimg2")
                        #print(imgURL)
                        if 'youtube' in imgURL: #skip youtube link
                            continue
                        '''
            except:
                with open('errmina.txt', 'a') as mysavedata:
